...
// `score / totalBlocks`, mirroring the approximate prefix-cache scorer's
// `matched_blocks / total_blocks` semantics. Pods missing from the score map
// (no hits) get 0.0; cold clusters (totalBlocks == 0) score 0.0 across the
// board.
//
// Replaces the prior min-max normalization, which had two real problems:
//   - cold cluster used to return 1.0 for every pod (full-strength noise)
//   - small absolute hits used to be stretched up to 1.0 if best-in-set
//
// Tier weights (e.g. GPU=1.0, CPU=0.8) flow through unchanged because they
// are already incorporated by upstream scoring; the cap below saturates the
