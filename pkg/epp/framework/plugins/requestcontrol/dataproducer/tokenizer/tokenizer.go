...
type tokenizerPluginConfig struct {
	...
	// Estimate selects the tokenizer-free estimation backend (byte-packing).
	// Mutually exclusive with 'vllm' and 'udsTokenizerConfig'; 'modelName' is
...
// estimateConfig configures the estimation backend. Its presence selects the
// backend; it has no parameters yet.
...
// configured backend. vllm (real /render) is the default; udsTokenizerConfig is
// the deprecated gRPC-over-UDS renderer; estimate is the tokenizer-free
// byte-packing backend.
...
// Produce derives the tokenized, KV-cache-affecting representation of the
// request via the configured backend and stores it on
// InferenceRequestBody.TokenizedPrompt. Returns an error when the backend
// fails; the caller (Director) decides the policy (currently: log and
// continue). If the request already carries a TokenizedPrompt, it is left
// unchanged.
